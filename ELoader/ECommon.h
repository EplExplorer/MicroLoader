...
#define		LBS_LIB_INFO2      		    (1 << 7)
	// 本信息结构实际上是否为LIB_INFO2
#define LBS_IDE_PLUGIN                  (1 << 8)
	// 本支持库是否为易语言IDE插件. 注意: 只有设置了此标志的支持库,才会接收到来自易语言IDE的NL_IDE_READY和NL_RIGHT_POPUP_MENU_SHOW等相关通知.
	DWORD				m_dwState;
...
typedef struct
{
	...
	// 支持库授权信息
	LPTSTR m_szHardwareCode;  // // 提供给支持库购买者的本计算机硬件特征码,如无则为NULL或空文本.
	LPTSTR m_szBuyingTips;  // 提供给支持库购买者的提示信息,如无则为NULL或空文本.
	LPTSTR m_szBuyingURL;  // 如果不为NULL或空文本,则提供一个用作接受购买信息的URL,用户选择购买本支持库时系统将自动调用浏览器打开此URL,并将相关用户信息参数附在其后.
	LPTSTR m_szLicenseToUserName;  // 本支持库所授权给的用户名称,如果为NULL/空文本,说明当前使用者尚未得到授权.
}
